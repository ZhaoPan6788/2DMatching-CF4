module ModuleGeometry
    ...
    type Geometry
        ...
        integer(4), allocatable :: cell_load(:, :)

        integer(4), allocatable :: point_type(:, :)                 ! 每个格点的类型
        real(8), allocatable :: point_volume_diag(:, :)             ! 每个格点的有效体积 假设dz=dr
        real(8), allocatable :: point_inv_volume_diag(:, :)         ! 每个格点的有效体积的倒数 假设dz=dr
        real(8), allocatable :: point_volume_weighting(:, :)        ! 每个格点的有效体积 假设dz=dr
        real(8), allocatable :: point_inv_volume_weighting(:, :)    ! 每个格点的有效体积的倒数 假设dz=dr
    ...
    contains
        ...
        subroutine initGeometry(this, Nz, Nr)
            ...
            if (Nz > 1 .and. Nr > 1) then
                ...
                allocate(this%cell_load(this%Nz-1, this%Nr-1))

                allocate(this%point_type(this%Nz, this%Nr))
                allocate(this%point_volume_diag(this%Nz, this%Nr))
                allocate(this%point_inv_volume_diag(this%Nz, this%Nr))
                allocate(this%point_volume_weighting(this%Nz, this%Nr))
                allocate(this%point_inv_volume_weighting(this%Nz, this%Nr))
        ...
        subroutine checkGeometry(this, name)
            ...
            if (0 == rank) then
                ...
                call h5dumpcheck%write('cell_load', this%cell_load)

                call h5dumpcheck%write('point_type', this%point_type)
                call h5dumpcheck%write('point_volume_diag', this%point_volume_diag)
                call h5dumpcheck%write('point_inv_volume_diag', this%point_inv_volume_diag)
                call h5dumpcheck%write('point_volume_weighting', this%point_volume_weighting)
                call h5dumpcheck%write('point_inv_volume_weighting', this%point_inv_volume_weighting)
        ...
        subroutine destroyGeometry(this)
            ...
            if (allocated(this%cell_load)) deallocate(this%cell_load)

            if (allocated(this%point_type)) deallocate(this%point_type)
            if (allocated(this%point_volume_diag)) deallocate(this%point_volume_diag)
            if (allocated(this%point_inv_volume_diag)) deallocate(this%point_inv_volume_diag)
            if (allocated(this%point_volume_weighting)) deallocate(this%point_volume_weighting)
            if (allocated(this%point_inv_volume_weighting)) deallocate(this%point_inv_volume_weighting)
