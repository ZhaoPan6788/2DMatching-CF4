Module ModuleFieldEM
	...
	Type FieldEM			! Definition of electromagnetic field
		...
		Real(8), Allocatable :: Ez(:, :), Er(:, :), Et(:, :)		! [Lz, Lr], Field in z, r, theta direction
		Real(8), Allocatable :: Bz(:, :), Br(:, :), Bt(:, :)
	...
	contains
        ...
        procedure, private :: DumpFieldEMDAT
	...
	contains
        ...
        subroutine DestroyFieldEM(FG)
            ...
            if (Allocated(FG%Er)) Deallocate(FG%Er)
            if (Allocated(FG%Et)) Deallocate(FG%Et)
            if (Allocated(FG%Bz)) Deallocate(FG%Bz)
            if (Allocated(FG%Br)) Deallocate(FG%Br)
            if (Allocated(FG%Bt)) Deallocate(FG%Bt)
