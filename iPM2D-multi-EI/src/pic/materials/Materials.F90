module ModuleMaterials
    ...
    type BoundaryOne
        ...
        integer(4), allocatable :: ih(:, :)

    end type BoundaryOne
    ...
    type Materials
        ...
        type(BoundaryOne), allocatable :: BO(:)
    ...
    contains
        ...
        procedure :: updataMB => updateMaterialBonundary
...
contains
    ...
    subroutine initMaterials(this, Nz, Nr, dielectric_num, metals_num, name)
        ...
        if (Nz > 0 .and. Nr > 0 .and. metals_num >= 0 .and. dielectric_num >= 0) then
            ...
            if (this%metal_count > 0) allocate(this%BO(this%metal_count))
    ...
    subroutine destroyMaterials(this)
        ...
        if (allocated(this%BO)) then
            do i = 1, this%metal_count
                if (allocated(this%BO(i)%iv)) deallocate(this%BO(i)%iv)
                if (allocated(this%BO(i)%ih)) deallocate(this%BO(i)%ih)
            end do
    ...
    subroutine updateMaterialBonundary(this, Geom)          ! 仅内部边界
        class(Materials), intent(inout) :: this
        type(Geometry), intent(in) :: Geom
        integer(4) :: i, j, k, nn, mlower, mupper

        if (this%metal_count > 0) then
            do k = 1, this%metal_count
                this%BO(k)%lv = 0
                this%BO(k)%lh = 0
                if (allocated(this%BO(k)%iv)) deallocate(this%BO(k)%iv)
                if (allocated(this%BO(k)%ih)) deallocate(this%BO(k)%ih)

                ! v
                do i = 1, Geom%Nz-1
                    do j = 1, Geom%Nr-2
                        mlower = Geom%cell_material(i, j)
                        mupper = Geom%cell_material(i, j+1)

                        if ((mlower == k .and. mupper <= 0) .or. (mlower <= 0 .and. mupper == k)) then
                            this%BO(k)%lv = this%BO(k)%lv + 1
                        end if
                    end do
                end do
                ...
                do i = 1, Geom%Nz-2
                    do j = 1, Geom%Nr-1
                        mlower = Geom%cell_material(i, j)
                        mupper = Geom%cell_material(i+1, j)

                        if ((mlower == k .and. mupper <= 0) .or. (mlower <= 0 .and. mupper == k)) then
                            this%BO(k)%lh = this%BO(k)%lh + 1
                        end if
                    end do
                end do

                if (this%BO(k)%lv > 0) allocate(this%BO(k)%iv(this%BO(k)%lv, 3))
                if (this%BO(k)%lh > 0) allocate(this%BO(k)%ih(this%BO(k)%lh, 3))
                ...
                do i = 1, Geom%Nz-1
                    do j = 1, Geom%Nr-2
                        ...
                        if (mlower == k .and. mupper <= 0) then
                            nn = nn + 1
                            this%BO(k)%iv(nn, 1) = i
                            this%BO(k)%iv(nn, 2) = j
                            this%BO(k)%iv(nn, 3) = 1

                        else if (mlower <= 0 .and. mupper == k) then
                            nn = nn + 1
                            this%BO(k)%iv(nn, 1) = i
                            this%BO(k)%iv(nn, 2) = j
                            this%BO(k)%iv(nn, 3) = -1
    ...
    end subroutine updateMaterialBonundary
